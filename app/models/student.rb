  ...
  def to_json
    {
      ...
      type: role,
      gender: gender && gender.humanize.split.map(&:capitalize).join(' '),
      race: race && race.humanize.split.map(&:capitalize).join(' '),
      major: major && major.humanize.split.map(&:capitalize).join(' '),
      degree_type: degree_type && degree_type.humanize.split.map(&:capitalize).join(' '),
      grad_year: grad_year,
      citizenship: citizenship && citizenship.humanize.split.map(&:capitalize).join(' '),
